...
-(void) createAsteroids
{
    ...
    int timer = arc4random() % 15000;//makes coins appear randomly
        ...
        if ([coins count] > 0)
        {
            ...
            if (coinHelper.position.x < (ship.position.x + 40) && coinHelper.position.x > ship.position.x - 40)
            {
                if (coinHelper.position.y < (ship.position.y + 60) && coinHelper.position.y > ship.position.y - 80)
                {
                    ...
                    newScoreLevel4++;
                    [self updateScore:newScoreLevel4];
                }
            }
        }
    }
...
- (void)updateScore:(int)newScoreLevel4
{
    [scoreLabel setString: [NSString stringWithFormat:@"%d", newScoreLevel4]];
}

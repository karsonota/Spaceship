...
-(void) createAsteroids
{
    ...
    int timer = arc4random() % 50;//makes coins appear randomly
        ...
        if ([coins count] > 0)
        {
            ...
            if (coinHelper.position.x < (ship.position.x + 40) && coinHelper.position.x > ship.position.x - 40)
            {
                if (coinHelper.position.y < (ship.position.y + 60) && coinHelper.position.y > ship.position.y - 80)
                {
                    ...
                    newScoreLevel5++;
                    [self updateScore:newScoreLevel5];
                }
            }
        }
    }
...
- (void)updateScore:(int)newScoreLevel5
{
    [scoreLabel setString: [NSString stringWithFormat:@"%d", newScoreLevel5]];
}
